...
from backend.services.email_scanner import scan_emails, scan_emails_streaming, scan_single_email
...
def scan_text():
    ...
    if "\n\n" in email_text:
        ...
        if any(h in parts[0] for h in ['From:', 'Subject:', 'Date:', 'Received:']):
            raw_headers = parts[0]
            raw_body = parts[1]
    ...
    loop = asyncio.new_event_loop()
    result = loop.run_until_complete(scan_single_email(email_data, 0))
    loop.close()

    predictor = get_ml_predictor()
    result['model_loaded'] = predictor.is_email_model_loaded()
